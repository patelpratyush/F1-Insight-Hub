...
const SectorAnalysis: React.FC<SectorAnalysisProps> = ({ driver1, driver2 }) => {
  ...
  const sectorDeltas = {
    sector_1: (driver2.sectorTimes.sector_1 || 0) - (driver1.sectorTimes.sector_1 || 0),
    sector_2: (driver2.sectorTimes.sector_2 || 0) - (driver1.sectorTimes.sector_2 || 0),
    sector_3: (driver2.sectorTimes.sector_3 || 0) - (driver1.sectorTimes.sector_3 || 0),
  };
  ...
  const deltaData = [
    { sector: 'S1', delta: sectorDeltas.sector_1, winner: sectorDeltas.sector_1 < 0 ? driver1.driver : driver2.driver },
    { sector: 'S2', delta: sectorDeltas.sector_2, winner: sectorDeltas.sector_2 < 0 ? driver1.driver : driver2.driver },
    { sector: 'S3', delta: sectorDeltas.sector_3, winner: sectorDeltas.sector_3 < 0 ? driver1.driver : driver2.driver },
  ];
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              <tr className="border-b border-gray-700">
                <td className="py-2">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <span className="text-white font-medium">{driver1.driver}</span>
                  </div>
                </td>
              ...
              <tr className="border-b border-gray-700">
                <td className="py-2">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <span className="text-white font-medium">{driver2.driver}</span>
                  </div>
                </td>
                <td className="text-center text-white py-2">
              ...
              <tr>
                ...
                <td className={`text-center py-2 font-bold ${Math.abs(sectorDeltas.sector_1) < 0.001 ? 'text-gray-400' : sectorDeltas.sector_1 < 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {formatDelta(sectorDeltas.sector_1)}
                </td>
                <td className={`text-center py-2 font-bold ${Math.abs(sectorDeltas.sector_2) < 0.001 ? 'text-gray-400' : sectorDeltas.sector_2 < 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {formatDelta(sectorDeltas.sector_2)}
                </td>
                <td className={`text-center py-2 font-bold ${Math.abs(sectorDeltas.sector_3) < 0.001 ? 'text-gray-400' : sectorDeltas.sector_3 < 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {formatDelta(sectorDeltas.sector_3)}
                </td>
                <td className={`text-center py-2 font-bold ${driver1.lapTime < driver2.lapTime ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700">
        <h3 className="text-lg font-bold text-white mb-4">Sector Time Comparison</h3>
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={sectorComparisonData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <YAxis 
                ...
                label={{ value: 'Time (s)', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fill: '#9CA3AF' } }}
              />
      ...
      <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700">
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={deltaData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
              <XAxis 
                dataKey="sector"
                axisLine={false}
        ...
        <div className="mt-4 text-sm text-gray-400">
          Positive values indicate {driver2.driver} is slower, negative values indicate {driver1.driver} is slower.
        </div>
      </div>

      {/* Sector Analysis Summary */}
      <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700">
        <h3 className="text-lg font-bold text-white mb-4">Sector Analysis Summary</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {['sector_1', 'sector_2', 'sector_3'].map((sector, index) => {
            ...
            return (
              <div key={sector} className="p-4 bg-gray-700/30 rounded-lg">
