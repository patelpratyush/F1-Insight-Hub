...
const ChampionshipPressureChart: React.FC<ChampionshipPressureProps> = ({
  ...
  const calculateChampionshipPressure = (standings: DriverStanding[]): PressureData[] => {
    ...
    return standings.map((driver, index) => {
      ...
      const pointsPerRaceNeeded = gapToLeader > 0 ? gapToLeader / safeRemainingRaces : 0;
      
      // Mathematical possibility check
      const mathematicallyPossible = driver.points + maxRemainingPoints > leaderPoints || driver.position === 1;
      ...
      if (driver.position === 1) {
        ...
        const gapToSecond = standings[1] ? driver.points - standings[1].points : 50;
        const defenseScore = Math.max(0, 100 - (gapToSecond / maxRemainingPoints) * 100);
        const raceProgress = ((currentRound || 0) / Math.max(1, totalRounds || 24)) * 100;
        ...
        if (!mathematicallyPossible) {
          ...
          pressureScore = Math.max(0, 100 - (gapRatio * 100) - positionPenalty) * urgencyMultiplier;
        }
      }
      ...
      if (mathematicallyPossible) {
        if (driver.position === 1) {
          championshipOdds = Math.min(95, 50 + (driver.points - (standings[1]?.points || 0)) / 10);
        } else {
      ...
      return {
        ...
        championshipOdds: isFinite(championshipOdds) ? Math.round(championshipOdds) : 0,
        pointsPerRaceNeeded: isFinite(pointsPerRaceNeeded) ? Math.round(pointsPerRaceNeeded * 10) / 10 : 0
      };
    });
  };
  
  const pressureData = calculateChampionshipPressure(standings);
  const mathematicalContenders = pressureData.filter(d => d.mathematicallyPossible);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-gray-800/50 border-gray-700">
        <CardHeader>
          ...
          <CardDescription className="text-gray-400">
            Real-time championship pressure based on current standings and remaining races
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="text-center p-4 bg-gradient-to-br from-blue-600/20 to-blue-800/20 rounded-lg border border-blue-600/20">
              <div className="text-2xl font-bold text-blue-400">{Math.max(0, remainingRaces || 0)}</div>
            ...
            <div className="text-center p-4 bg-gradient-to-br from-green-600/20 to-green-800/20 rounded-lg border border-green-600/20">
              <div className="text-2xl font-bold text-green-400">{mathematicalContenders.length}</div>
            ...
            <div className="text-center p-4 bg-gradient-to-br from-purple-600/20 to-purple-800/20 rounded-lg border border-purple-600/20">
              <div className="text-2xl font-bold text-purple-400">{Math.max(0, (remainingRaces || 0) * 26)}</div>
            ...
            <div className="text-center p-4 bg-gradient-to-br from-yellow-600/20 to-yellow-800/20 rounded-lg border border-yellow-600/20">
              <div className="text-2xl font-bold text-yellow-400">{Math.round(((currentRound || 0) / Math.max(1, totalRounds || 24)) * 100)}%</div>
      ...
      <Card className="bg-gray-800/50 border-gray-700">
        <CardHeader>
          <CardTitle className="text-white">Championship Pressure Levels</CardTitle>
          <CardDescription className="text-gray-400">
            Pressure intensity from 0-100 based on position, points gap, and remaining races
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="h-80">
              ...
              <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-gray-800 border border-gray-600 rounded-lg p-3 shadow-lg">
                          ...
                          <p className="text-blue-400">Position: P{data.position}</p>
                          <p className="text-green-400">Points: {data.points}</p>
                          <p className="text-purple-400">Pressure: {data.pressure}/100</p>
                ...
                <Bar dataKey="pressure" radius={[4, 4, 0, 0]}>
                  {chartData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={getPressureColor(entry.level, entry.position)} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
      ...
      <Card className="bg-gray-800/50 border-gray-700">
        <CardHeader>
          <CardTitle className="text-white">Driver-by-Driver Analysis</CardTitle>
          <CardDescription className="text-gray-400">
        ...
        <CardContent>
          <div className="space-y-4">
            {pressureData.slice(0, 6).map((driver, index) => (
              <div key={driver.driver} className="p-4 bg-gray-700/30 rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm ${
                      driver.position === 1 ? 'bg-yellow-500' : 
                      driver.position === 2 ? 'bg-gray-400' : 
                      driver.position === 3 ? 'bg-amber-600' : 'bg-gray-600'
                    }`}>
                      {driver.position}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center space-x-4">
                    <Badge 
                      variant="outline" 
                      className={`${
                        driver.pressureLevel === 'Critical' ? 'text-red-400 border-red-400' :
                        driver.pressureLevel === 'High' ? 'text-orange-400 border-orange-400' :
                        driver.pressureLevel === 'Medium' ? 'text-yellow-400 border-yellow-400' :
                        'text-green-400 border-green-400'
                      }`}
                    >
                      <span className="mr-1">{getPressureIcon(driver.pressureLevel, driver.position)}</span>
                      {driver.pressureLevel}
                    </Badge>
                    <div className="text-right">
                      <div className="text-lg font-bold text-white">{driver.points} pts</div>
                      {driver.gapToLeader > 0 && (
                        <div className="text-sm text-red-400">-{driver.gapToLeader}</div>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      ...
                      />
                      <span className="text-white font-medium w-12">{driver.pressureScore}/100</span>
                    </div>
                  </div>
                  <div>
                    <span className="text-gray-400">Championship Odds:</span>
                    <div className="text-white font-medium mt-1">{driver.championshipOdds}%</div>
                  </div>
                  <div>
                    <span className="text-gray-400">Points/Race Needed:</span>
                    <div className="text-white font-medium mt-1">
                      {(driver.pointsPerRaceNeeded > 0 && isFinite(driver.pointsPerRaceNeeded)) ? driver.pointsPerRaceNeeded : '0'}
                    </div>
                  </div>
                  <div>
                    <span className="text-gray-400">Mathematical Chance:</span>
                    <div className={`font-medium mt-1 ${driver.mathematicallyPossible ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <Card className="bg-gray-800/50 border-gray-700">
        <CardHeader>
          <CardTitle className="text-white">Championship Odds Distribution</CardTitle>
          <CardDescription className="text-gray-400">
              ...
              <PieChart>
                ...
                >
                  {oddsData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={getPressureColor('Medium', entry.position)} />
                  ))}
                </Pie>
                <Tooltip 
                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-gray-800 border border-gray-600 rounded-lg p-3 shadow-lg">
                          ...
                          <p className="text-green-400">Championship Odds: {data.value}%</p>
                          <p className="text-blue-400">Current Position: P{data.position}</p>
                        </div>
                      );
                    }
                    return null;
