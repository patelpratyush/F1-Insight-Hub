...
const DriverComparisonTelemetry: React.FC<DriverComparisonProps> = ({
  ...
  const getChartData = () => {
    ...
    const driver1Speed = interpolateData(
      driver1.telemetry[xAxisKey],
      driver1.telemetry.speed,
      alignedXValues,
    );
    const driver2Speed = interpolateData(
      driver2.telemetry[xAxisKey],
      driver2.telemetry.speed,
      alignedXValues,
    );
    const driver1Throttle = interpolateData(
      driver1.telemetry[xAxisKey],
      driver1.telemetry.throttle,
      alignedXValues,
    );
    const driver2Throttle = interpolateData(
      driver2.telemetry[xAxisKey],
      driver2.telemetry.throttle,
      alignedXValues,
    );
    const driver1Brake = interpolateData(
      driver1.telemetry[xAxisKey],
      driver1.telemetry.brake,
      alignedXValues,
    );
    const driver2Brake = interpolateData(
      driver2.telemetry[xAxisKey],
      driver2.telemetry.brake,
      alignedXValues,
    );
    const driver1Gear = interpolateData(
      driver1.telemetry[xAxisKey],
      driver1.telemetry.gear,
      alignedXValues,
    );
    const driver2Gear = interpolateData(
      driver2.telemetry[xAxisKey],
      driver2.telemetry.gear,
      alignedXValues,
    );
    const driver1RPM = driver1.telemetry.rpm
      ? interpolateData(
          driver1.telemetry[xAxisKey],
          driver1.telemetry.rpm,
          alignedXValues,
        )
      : [];
    const driver2RPM = driver2.telemetry.rpm
      ? interpolateData(
          driver2.telemetry[xAxisKey],
          driver2.telemetry.rpm,
          alignedXValues,
        )
      : [];
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <div className="text-sm text-green-400">
                {formatLapTime(driver1.lapTime)}
              </div>
            </div>
          </div>
        </div>

        <div className="bg-white/5 p-4 rounded-[24px] border border-white/5 text-center">
          <div className="text-lg font-bold text-white">VS</div>
          <div className="text-sm text-gray-400">
            Gap: {Math.abs(driver1.lapTime - driver2.lapTime).toFixed(3)}s
          </div>
          <div className="text-xs text-gray-500">
            {driver1.lapTime < driver2.lapTime
              ? driver1.driver
              : driver2.driver}{" "}
            faster
          </div>
      ...
      <div className="bg-transparent border-t border-l border-white/5 bg-gradient-to-br from-white/[0.02] to-transparent p-6 rounded-[40px] shadow-2xl">
            ...
            <ComposedChart data={chartData}>
              ...
              <Tooltip
                content={({ active, payload, label }) => {
                  if (active && payload && payload.length) {
                    const delta =
                      chartData.find((d) => d[xAxisKey] === label)
                        ?.speed_delta || 0;
                    const fasterDriver =
                      delta > 0 ? driver1.driver : driver2.driver;
                    return (
                      <div className="bg-[#111111] border border-white/10 rounded-[16px] p-4 shadow-2xl">
                        <p className="text-white font-medium">
                          {viewMode === "distance"
        ...
        <div className="mt-4 text-sm text-gray-400">
          Positive values indicate {driver1.driver} is faster, negative values
          indicate {driver2.driver} is faster.
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-green-400">
              {(
                (driver1.telemetry.throttle.filter((t) => t === 100).length /
                  driver1.telemetry.throttle.length) *
                100
              ).toFixed(1)}
              %
            </div>
            <div className="text-sm text-gray-300">
              {driver1.driver} Full Throttle
            </div>
          </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-green-400">
              {(
                (driver2.telemetry.throttle.filter((t) => t === 100).length /
                  driver2.telemetry.throttle.length) *
                100
              ).toFixed(1)}
              %
            </div>
            <div className="text-sm text-gray-300">
              {driver2.driver} Full Throttle
            </div>
          </div>
