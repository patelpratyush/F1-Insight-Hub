...
const TeamProfile = () => {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["team-profile", name],
    queryFn: async () => {
      const response = await fetch(
        `${API_BASE}/api/f1/team/${encodeURIComponent(name || "")}?years=6`,
      );
      if (!response.ok) throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      const json = await response.json();
      return json as TeamProfileData;
    },
    enabled: !!name,
            ...
            {data && (
              <div className="flex flex-col gap-12">
                <div className="flex items-start justify-between gap-6">
                  ...
                  <button
                    type="button"
                    onClick={() => toggleFavorite("team", data.name)}
                    aria-label={
                      isFavorite("team", data.name)
                  ...
                  >
                    <Star
                      className={`h-6 w-6 ${isFavorite("team", data.name) ? "fill-yellow-400 text-yellow-400" : "text-white/40"}`}
                    />
                  </button>
                </div>
